...
//print error "a" until read a valid word
pair<string, string> get_a_word(){
    lastSymbolNum=symbolNum;
    //initialize
    lexical_str.clear();
    lexical_type.clear();
    skip_blank();
    ...
    if (c == '\'') {//character
        ...
        if (c == '\'' && lexical_str.length() == 1) {//true
            ...
            if(c!='\''){
                while(c!='\'')c = file_getChar();
            }
            c=file_getChar();
            file_error << lastSymbolNum << " a" << endl;
            return PAIR("CHARCON","ERROR");
        }
    } else if (c == '\"') {//string
        while ((c = file_getChar()) && is_string(c)) {
            if(c == '\\') lexical_str.append(1,'\\');
            lexical_str.append(1, c);
        }
        if (c == '\"' && lexical_str.length() >= 1) {//true
            lexical_type = "STRCON";
